...
import entityBasicClasses.AvgEvaluationResultsType;
import entityBasicClasses.EvaluationResultsType;
import entityBasicClasses.ItemForPackage_v2;
import entityBasicClasses.Measures_v2;
...
public class F_Evaluation {
    ...
    static public int[] evaluationUsersId = movielens; //It should be changed to specific dataset
    ...
    public void runEvaluation() throws IOException {
        ...
        //evaluationOfaRangeOfUsers(10, 10, 5);
        ...
        System.out.println("\nOptimal - time:" + (System.currentTimeMillis() - startTime) + "msec");
    }
    ...
    public void evaluationOfUsers(int[] users, int folds) throws IOException {
        ...
        for (int u = 0; u < len; u++) {
            System.out.println("userId: " + users[u]);
    ...
    public void evaluationOfRandomUsers(int fromUser, int toUser, int folds) throws IOException {
        ...
        for (int u = fromUser; u < toUser + 1; u++) {
            ...
            if (A_Start.ratingsPerUser_hashTable.get(u).getNumOfRatings() > 500 && u != 438 && u != 482) {
    ...
    public void evaluationPerUser(int userId, int k_folds) {
        try {
            ...
            for (int f = 1; f < k_folds + 1; f++) {
                ...
                double[] runningTime = e.createPackages();
                runningTime_listOfFolds.add(runningTime);
                //
                //e.printSolution();

                RatingsCost_OfPackage ratingsCostPerPackage = e.RatingsAndCostPerPackage();
                ...
                Measures_v2 measures = eval.getMeasures(minRatingForModifiedPrecision);
                double[] precisionPerPackage = eval.precisionPerPackage(measures, C_ProblemModeling.numOfPackages);
                precisionPerPackage_listOfFolds.add(precisionPerPackage); //list for all the folds
                double[] modPrecisionPerPackage = eval.modPrecisionPerPackage(measures, C_ProblemModeling.numOfPackages);
            ...
            System.out.println("error in userId: " + userId);
            e.printStackTrace();
            return;
        }
    }
    ...
    public void printAvgEvaluationResults(AvgEvaluationResultsType avg_evaluationResults) {
        try {
            ...
            String[] row2 = {"Avg:",
                ...
                ((int) avg_evaluationResults.getAvg_runningTime()) + " msec"};
            System.out.format("%-15s%-16s%-15s%-15s%-15s%-15s\n", row2);
        } catch (Exception e) {
            e.printStackTrace();
            return;
        ...
        try {
            ...
            for (int u = 0; u < size; u++) {
                ...
                String[] row2 = {"Avg:",
                   ...
                   ((int) er.getAvg_runningTime()) + " msec"};
                System.out.format("%-15s%-16s%-15s%-15s%-15s%-15s\n", row2);
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//avg  per user of all the packages and all the folds
